...
class FusionRAGCacheManager:
    ...
    def new_query(self, prompt, force_full=False, snapshot_path=None):
        ...
        query_final_q = self._compute_query_final_layer_q(query, context_length, stats)
        
        # Step 2: Get final layer keys and lengths for all chunks
        chunk_final_keys, chunk_lengths, chunk_original_position_ranges = self.cache.get_final_layer_keys_and_lengths(chunks)
        ...
        stats["recompute_ratio_actual"] = len(recompute_indices) / context_length if context_length > 0 else 0

        # Step 4: Build blended cache with pre-computed indices
        with Timer("reuse_cache", stats):
            cache = self.cache.reuse_cache(
                chunks, self.llm, self.tokenizer, stats, recompute_indices,
