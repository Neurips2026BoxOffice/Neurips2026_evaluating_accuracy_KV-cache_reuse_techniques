...
class Baseline:
    ...
    def _direct_query_no_context(self, query: torch.Tensor, stats: dict, max_length: int = 50):
        ...
        for _ in range(max_length):
            outputs = self.llm(input_ids=input_ids, use_cache=True, output_attentions=False)
    ...
    def new_query(self, prompt: torch.Tensor, force_full=False, snapshot_path = None):
        stats = {}
        # Diego Method-A parity: baseline must prefill/generate from the exact
        # tokenized prompt, including <DSEP> separators and chat-template tokens.
        try:
...
class BaselineNosep:
    ...
    Ported from cache_reuse_benchmark/src/cachebend/ncf/zcf_v10.py — this
    is the canonical "baseline" used across paper2 (boxoffice, intro_lb,
    fr_extended). Per CLAUDE.md: `baseline ≡ BaselineNosep`.

    Flow per query:
        tokenized prompt (with DSEP, single BOS from chat template)
          → dedupe leading BOS defensively
          → split on DSEP (chunks_from_tokenss strips DSEP from output)
          → concat chunk tokens (no separator) for a single prefill
          → manual token-by-token decode of the last chunk (the query)
            reusing the prefill KV cache

    Differences vs. legacy `Baseline` (in this same file):
      • The original Baseline's force_full=True path forwarded the
        prompt with DSEP tokens included. BaselineNosep always runs
        DSEP-stripped — same token layout CacheBlend uses when it
        rebuilds chunk KV caches.
      • Fixes the double-BOS injection: chat templates emit BOS as text
        and tokenize to bos_token_id; a subsequent tokenize(...,
        add_special_tokens=True) prepends a second BOS.
        _dedupe_leading_bos collapses consecutive BOS ids at the start
        of the input.
      • Only DSEP (tokenizer.sep_token_id) defines chunk boundaries —
    ...
    def tokenize(self, doc: str, use_special_tokens: bool = False) -> torch.Tensor:
        # Default add_special_tokens=False: the chat template (applied
        # inside to_str_prompt) already injected BOS. Setting True here
        # would inject a second one — _dedupe_leading_bos cleans it up
        # regardless.
        return self.tokenizer(
            doc, return_tensors="pt", add_special_tokens=use_special_tokens
        )["input_ids"][0].to(torch.int64)
    ...
    def _dedupe_leading_bos(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        if bos_id is None or tokens.numel() < 2:
            return tokens
        i = 0
        while (i + 1 < tokens.numel()
               and int(tokens[i]) == bos_id
               and int(tokens[i + 1]) == bos_id):
            i += 1
        return tokens[i:] if i > 0 else tokens

    @torch.no_grad()
    def _direct_query_no_context(self, query: torch.Tensor, stats: dict, max_length: int = 50):
        # Threads past_key_values across decode steps. Without this,
        # every step after the first re-encodes a single token with no
        # history → "predict next token from a token in isolation"
        # (typically high-frequency garbage).
        # Stop conditions: EOS OR a "\n\n" sequence in the accumulating
        # decoded text — mirrors vLLM's stop=["\n\n"] (used by the v8
        # gate's T1a/T1b), so this path's predictions are directly
    ...
    def new_query(self, prompt: torch.Tensor, force_full: bool = False,
        ...
        """force_full kept for API parity but ignored — this class
        always runs the DSEP-stripped path. max_new_tokens caps greedy
        decoding length (default 50)."""
        stats = {}
