...
def ensure_tokenizer_model_alignment(llm, tokenizer) -> None:
    """Resize embeddings when added special tokens, such as <DSEP>, are in the prompt."""
    try:
    ...
    if tok_size > vocab_size:
        try:
            llm.resize_token_embeddings(tok_size, mean_resizing=False)
        except TypeError:
            llm.resize_token_embeddings(tok_size)
...
def build_model_eager(model_path):
    ...
    if model_type not in {"ministral", "mistral"}:
        config.sliding_window = None
...
def _attention_impl_candidates() -> List[str]:
    requested = os.getenv("CACHEBEND_ATTENTION_IMPL", "flash_attention_2").strip()
    if requested.lower() in {"", "auto"}:
        requested = "flash_attention_2"
    if requested.lower() in {"sdpa", "flash_attention_2", "eager"}:
...
def build_model_sdpa(model_path, torch_dtype: str = "bfloat16", attention_impl: str | None = None):
    config = AutoConfig.from_pretrained(model_path)
    dtype = resolve_torch_dtype(torch_dtype)
    candidates = [attention_impl] if attention_impl else _attention_impl_candidates()
    last_error = None
    for impl in candidates:
        ...
        try:
            ...
            llm.eval()
            print(f"cutils: model built from {model_path=} with {impl} attention and dtype={dtype}")
            return llm
        except Exception as exc:
            last_error = exc
            print(f"cutils: attention_impl={impl!r} failed for {model_path=}: {exc!r}")
    raise RuntimeError(f"Could not load {model_path=} with attention implementations {candidates}") from last_error
...
def build_tokenizer(model_path):
    ...
    # Diego-style runs keep <DSEP> in the exact tokenized prompt. Call
    # ensure_tokenizer_model_alignment(model, tokenizer) after model creation.
