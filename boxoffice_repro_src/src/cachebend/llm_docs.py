...
def apply_rotary_pos_emb_inplace(
    ...
    """
    Optimized RoPE function for Ascend NPU, respecting PyTorch's autograd rules.
    - Avoids in-place modification of views.
    """
    # Create the broadcastable cos and sin tensors
    cos = cos.unsqueeze(unsqueeze_dim)
    sin = sin.unsqueeze(unsqueeze_dim)

    # Calculate the rotated tensor out-of-place. This is a single, efficient operation.
    q_rot = rotate_half(q)
    k_rot = rotate_half(k)

    # Perform the calculations. This is where the core fused operation happens.
    # The output of this operation is the final tensor.
    # The form is (q * cos) + (q_rot * sin * flag)
    q_embed = (q * cos).addcmul_(q_rot, sin, value=flag)
    k_embed = (k * cos).addcmul_(k_rot, sin, value=flag)
...
def apply_rotary_pos_emb(
    ...
    cos = cos.unsqueeze(unsqueeze_dim)
    sin = sin.unsqueeze(unsqueeze_dim)
    q_embed = (q * cos) + flag * (rotate_half(q) * sin)
    k_embed = (k * cos) + flag * (rotate_half(k) * sin)
    return q_embed, k_embed
...
def eager_attention_forward_fused(
    ...
    # When run on an NPU, this function automatically dispatches to
    # a highly optimized, fused kernel.
    attn_output = torch.nn.functional.scaled_dot_product_attention(
        query,
        key_states,
        value_states,
        attn_mask=attention_mask,
        dropout_p=0.0, # Dropout is typically handled here
        is_causal=False # Set to True for causal attention --> but we do pass a mask explicitly
    )
...
def _fill_buffer_async(fetch_plan, layer_idx, k_buffer, v_buffer):
    """
...
def layer_prefetch_generator2(fetch_plan, doc_ids, dtype, num_layers, stats):
    """
    Double-Buffered (Ping-Pong) Generator.
    Yields layer N, while asynchronously fetching layer N+1 into the reserve buffer.
    """
    
    #determine Shape from the first source
    #peek at layer 0 of the first source to get heads/dim
    #IMOPRTNAT: We fetch to CPU first just to check shape metadata cheaply if needed, 
    # but my guess is that getting the source object property is enough.
    ref_source = fetch_plan[0]["k_source"]
    
    # Logic to handle your ChunkWithState object or standard list
    if hasattr(ref_source, "states"):
        # Access [0][0] -> k, layer 0. Shape: [batch, n_heads, seq, dim]
        ref_shape = ref_source.states[0][0].shape
    else:
        ref_shape = ref_source[0].shape
        
    # shape is [Batch, Heads, Doc_Len, Head_Dim]
    #Important!!!: doc_ids.shape[1] is the total sequence length we build
    buffer_shape = (doc_ids.shape[0], ref_shape[-3], doc_ids.shape[1], ref_shape[-1])
    device = doc_ids.device

    # Low-mem fallback: avoid 4x buffer allocation for very long contexts.
    if _should_use_low_mem_prefetch(buffer_shape, dtype, device, stats):
        ...
        for layer_id in range(1, num_layers):
            with Timer(f"prefetch_L{layer_id}_seq", stats=stats):
                _fill_buffer_async(fetch_plan, layer_id, k_buf, v_buf)
    ...
    # This removes a 'malloc' overhead from loop
    # for some reason, using empty here creates problems, ugh
    k_bufs = [
    ...
    # 'ping' indicates which buffer is being Computed (yielded)
    # 'pong' (1-ping) indicates which buffer is being Prefetched
    ping = 0 

    #bootstrap: prefetch Layer 1 immediately into buffer 0
    #Layer 0 is processed via standard embedding lookup, so we start cache at Layer 1
    if num_layers > 0:
        with Timer("prefetch_L0_init", stats=stats):
             
             #  need to have Layer 1 ready when the generator is called first
             
             _fill_buffer_async(fetch_plan, 1, k_bufs[ping], v_bufs[ping])
             torch.npu.synchronize() 
    ...
    for layer_id in range(1, num_layers):
        ...
        # buffer 'ping' should ready 
        # yield it to the main loop to be used.
        curr_k = k_bufs[ping]
        curr_v = v_bufs[ping]
        
        #for debug purposes
        if False:
            print(f"DEBUG GEN: Yielding L{layer_id} BufferSum: {curr_k.float().sum().item()}")
        
        # DEBUG: Check if buffer is actually populated
        if isdbg() and layer_id == 1:
             if curr_k.abs().sum() == 0:
                 print(f"[CRITICAL FAIL]Ao! Generator yielding EMPTY BUFFER for Layer {layer_id}")
        
        # Before yielding, trigger fetch for the NEXT layer into the 'pong' buffer
        next_layer = layer_id + 1
        pong = 1 - ping
        
        if next_layer < num_layers:
            #asynch\ fetch for the next iteration
            _fill_buffer_async(fetch_plan, next_layer, k_bufs[pong], v_bufs[pong])
...
def generator_build_blent_cache4(
    ...
    cache_generator = layer_prefetch_generator2(
        layer_fetch_plan, doc_ids, model.dtype, config.num_hidden_layers, stats
    )
...
def layer_prefetch_generator5(fetch_plan, doc_ids, dtype, num_layers, stats):
    ...
    """
    ref_source = fetch_plan[0]["k_source"]
    ref_shape = ref_source.states[0][0].shape if hasattr(ref_source, "states") else ref_source[0].shape
    ...
    # Ping-Pong Buffers
    k_bufs = [torch.zeros(buffer_shape, device=device, dtype=dtype),
              torch.zeros(buffer_shape, device=device, dtype=dtype)]
    v_bufs = [torch.zeros(buffer_shape, device=device, dtype=dtype),
              torch.zeros(buffer_shape, device=device, dtype=dtype)]
    
    ping = 0 

    # Prefetch Layer 0 history immediately
    with Timer("prefetch_L0_init", stats=stats):
         _fill_buffer_async(fetch_plan, 0, k_bufs[ping], v_bufs[ping])
         if hasattr(torch, "npu"): torch.npu.synchronize()

    # Generator Loop starts at 0
    for layer_id in range(0, num_layers):
        curr_k, curr_v = k_bufs[ping], v_bufs[ping]
...
def generator_build_blent_cache5(
    ...
    assert doc_ids.shape[0] == 1
    doc_embeds = model.embed_tokens(doc_ids)
    precomputed_recompute_indices = kwargs.get("precomputed_recompute_indices")
    precomputed_recompute_scores = kwargs.get("precomputed_recompute_scores")
    precomputed_selection_mode = kwargs.get("precomputed_selection_mode", "diffk")
    ...
    for layer_id, decoder_layer in enumerate(model.layers[:num_layers]):
        ...
        if layer_id + 1 < num_layers:
            ...
            if layer_id + 1 == 1:
                if precomputed_recompute_indices is not None:
                    recompute_indices = sorted(
                        {
                            int(x)
                            for x in precomputed_recompute_indices
                            if 0 <= int(x) < total_tokens
                        }
                    )
                    if stats is not None:
                        stats["mode"] = stats.get("mode") or "lmcache_online_preselected"
                        stats["num_selected"] = len(recompute_indices)
                        stats["recompute_selected_indices"] = recompute_indices
                else:
                    ...
                    with Timer("select_recompute_tokens", stats=stats):
                        recompute_indices, _, _ = _g5_select_recompute_tokens(
                            ...
                            blender=blender,
                            stats=stats,
                            external_scores=precomputed_recompute_scores,
                            selection_mode=precomputed_selection_mode,
                        )
...
def _g5_select_recompute_tokens(
    ...
    diff_per_token = torch.mean((fresh_k - rescaled_k) ** 2, dim=(0, 2))
    diff_per_token_masked = diff_per_token * valid_mask.to(device)
    selection_scores = diff_per_token
    ...
    if not is_lmcache_online:
        ...
        selector_mode = str(selection_mode).lower()
        if hit_indices and external_scores is not None and selector_mode in {"hybrid", "mix", "guarded"}:
            query_scores = torch.as_tensor(external_scores, device=device, dtype=torch.float32)
            if query_scores.numel() != total_tokens:
                query_scores = torch.zeros(total_tokens, device=device, dtype=torch.float32)
            diff_hit = diff_per_token[hit_indices].float()
            query_hit = query_scores[hit_indices].float().clamp_min(0.0)
            ...
            def _minmax(x: torch.Tensor) -> torch.Tensor:
                ...
                xmin = x.min()
                xmax = x.max()
                denom = (xmax - xmin).clamp_min(1e-12)
                return (x - xmin) / denom

            diff_norm = _minmax(diff_hit)
            query_norm = _minmax(query_hit)
            if selector_mode == "mix":
                query_epsilon = 0.5
                hybrid_hit = diff_norm * (query_epsilon + query_norm)
                selector_name = "lmcache_mix"
                selection_rule = (
                    "top R% cached-hit tokens by minmax(layer-1 diffK) * "
                    "(0.5 + minmax(final-layer query attention)); all miss tokens recomputed"
                )
            elif selector_mode == "guarded":
                query_epsilon = 0.0
                hybrid_hit = diff_norm * query_norm
                selector_name = "lmcache_guarded"
                selection_rule = (
                    "top R% cached-hit tokens by hybrid score unless >3% of the hit budget "
                    "would go to the first prompt chunk, then fall back to diffK; all miss tokens recomputed"
                )
            else:
                query_epsilon = 0.0
                hybrid_hit = diff_norm * query_norm
                selector_name = "lmcache_hybrid"
                selection_rule = (
                    "top R% cached-hit tokens by minmax(layer-1 diffK) * "
                    "minmax(final-layer query attention); all miss tokens recomputed"
                )
            hit_ratio_by_idx = {
                idx: ratio
                for (start, end, _cid, ratio) in chunk_boundaries
                if ratio < 1.0
                for idx in range(start, end)
            }
            guard_triggered = False
            guard_first_chunk_selected = 0
            guard_hit_budget = 0
            selected_hit_scores = hybrid_hit
            if selector_mode == "guarded":
                guard_hit_budget = int(len(hit_indices) * hit_ratio_by_idx[hit_indices[0]])
                first_hit_boundary = next(
                    (
                        (start, end)
                        for (start, end, _cid, ratio) in chunk_boundaries
                        if ratio < 1.0 and start == 0
                    ),
                    None,
                )
                if guard_hit_budget > 0 and first_hit_boundary is not None:
                    top_offsets = torch.topk(
                        hybrid_hit,
                        min(guard_hit_budget, hybrid_hit.numel()),
                    ).indices.tolist()
                    first_start, first_end = first_hit_boundary
                    guard_first_chunk_selected = sum(
                        1 for offset in top_offsets if first_start <= hit_indices[int(offset)] < first_end
                    )
                    guard_triggered = (guard_first_chunk_selected / guard_hit_budget) > 0.03
                if guard_triggered:
                    selected_hit_scores = diff_hit
            selection_scores = torch.zeros_like(diff_per_token, dtype=torch.float32)
            selection_scores[hit_indices] = selected_hit_scores.to(selection_scores.device)
            for offset, idx in enumerate(hit_indices):
                # Tie-break with a tiny diffK component for deterministic ordering.
                score = float(selected_hit_scores[offset].item()) + 1e-9 * float(diff_hit[offset].item())
                hit_deltas.append((idx, score, hit_ratio_by_idx[idx]))
            if stats:
                stats["mode"] = selector_name
                stats[f"{selector_name}_selection_rule"] = selection_rule
                stats[f"{selector_name}_external_scores_available"] = True
                stats[f"{selector_name}_diffk_mean"] = float(diff_hit.mean().item()) if diff_hit.numel() else 0.0
                stats[f"{selector_name}_query_mean"] = float(query_hit.mean().item()) if query_hit.numel() else 0.0
                stats[f"{selector_name}_query_epsilon"] = float(query_epsilon)
                stats[f"{selector_name}_score_mean"] = float(selected_hit_scores.mean().item()) if selected_hit_scores.numel() else 0.0
                if selector_mode == "guarded":
                    stats["lmcache_guarded_triggered"] = bool(guard_triggered)
                    stats["lmcache_guarded_first_chunk_selected_by_hybrid"] = int(guard_first_chunk_selected)
                    stats["lmcache_guarded_hit_budget"] = int(guard_hit_budget)
                    stats["lmcache_guarded_fallback"] = "diffk" if guard_triggered else "hybrid"
        else:
            for (start, end, cid, ratio) in chunk_boundaries:
                if ratio < 1.0:
                    for idx in range(start, end):
            ...
            if stats and selector_mode in {"hybrid", "mix", "guarded"}:
                stats[f"lmcache_{selector_mode}_external_scores_available"] = False
