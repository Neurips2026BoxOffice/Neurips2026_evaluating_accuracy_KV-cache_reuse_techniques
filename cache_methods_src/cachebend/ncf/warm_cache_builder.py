...
def export_contextualized_cache(
    ...
    if use_eager:
        ...
        try:
            llm = build_model_sdpa(model_path, dtype).to(device)
        except TypeError:
            llm = build_model_sdpa(model_path).to(device)
    llm.eval()
    ...
    prefix = "Answer the question based on the following text."
    dummy_query = "X"
    prefix_ids = tokenizer(prefix, add_special_tokens=False).input_ids
    query_ids = tokenizer(dummy_query, add_special_tokens=False).input_ids
    sep_ids = tokenizer(sep_token, add_special_tokens=False).input_ids or [tokenizer.eos_token_id]
    ...
    for row in tqdm(fusion_docs, desc="Building warm cache"):
        ...
        input_seq: List[int] = []
        input_seq.extend(prefix_ids)
        for ctx in row.get("top_k_contexts", []):
            ctx_tokens = [int(x) for x in ctx.get("tokens", [])]
            if not ctx_tokens:
                ctx_tokens = tokenizer(ctx.get("text", ""), add_special_tokens=False).input_ids
            input_seq.extend(sep_ids)
            input_seq.extend(ctx_tokens)
        input_seq.extend(sep_ids)
        input_seq.extend(ref_ids)
        input_seq.extend(sep_ids)
        input_seq.extend(query_ids)
...
def main() -> None:
    ...
    ap.add_argument("--dtype", default="float16", choices=["float16", "bfloat16", "float32", "fp16", "bf16", "fp32"])
