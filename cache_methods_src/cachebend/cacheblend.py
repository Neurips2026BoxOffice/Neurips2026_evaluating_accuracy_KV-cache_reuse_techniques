...
logger.setLevel(os.getenv("CACHEBLEND_LOG_LEVEL", "INFO").upper())
...
def _ranges_str(indices):
    """Compact representation of a list of ints as ranges: [0-99, 150-200, ...]"""
    if not indices:
        return "[]"
    ...
    for v in vals[1:]:
        if v == prev + 1:
            ...
            start = prev = v
    ranges.append(f"{start}-{prev}" if prev > start else str(start))
    return f"[{', '.join(ranges)}] ({len(vals)} tokens)"
    ...
    def _apply_tokens_all_queries(
        ...
        for qstart, qend in zip(
            ...
            if isdbg():
                ...
                logger.debug(f"top_indices: {_ranges_str(top_indices)}")
    ...
    def blend(
        ...
        if layer_id == 0:
            ...
            if isinstance(self.recompute_ratio,list):
                logger.debug(f"@ddi blending l1 at fixed positions: {_ranges_str(self.recompute_ratio)}")
            ...
            reuse_mask = torch.ones(
                retrieved_k.shape[token_dim],
                dtype=torch.bool,
                device=self.indexes_in_kv.device,
            )
            reuse_mask[self.indexes_in_kv] = False
            reuse_indices = mask_to_indices(reuse_mask)
            reuse_index_obj = create_index(retrieved_k.dim(), token_dim, reuse_indices)
            if ( self.positional_encoder is not None and self.reverse_positional_encoder is not None):
                ...
                if reuse_indices.numel() > 0:
                    if ROPE32:
                        rk_with_position[reuse_index_obj] = self.rescale32(
                            retrieved_k[reuse_index_obj],
                            original_positions[reuse_indices],
                            positions[reuse_indices],
                        )
                    else:
                        rk_with_position[reuse_index_obj] = self.rescale(
                            retrieved_k[reuse_index_obj],
                            original_positions[reuse_indices],
                            positions[reuse_indices],
                        )
